...
class BaseBehavior(object):
    ...
    def serialize(self):
        serobj = {'timestamp': self.date, 'relation': self.relation, 'attckids': self.attck_ids,
                  'behaviortype': self.getname(), 'value': self.get_value()}
        for _attr in self.__class__.CONTEXT:
...
class ProcessBehavior(BaseBehavior):
    ...
    def __str__(self):
        return '{} Endpoint({}) on {}: ({} {}) -{}-> ({} {})'.format(self.__class__.__name__, self.endpoint['uuid'],
                                                                     self.date, self.parent['image'],
                                                                     self.parent['cmdline'], self.relation,
                                                                     self.current['image'], self.current['cmdline'])

    def get_value(self):
        return '({} {}) -{}-> ({} {})'.format(self.parent['image'], self.parent['cmdline'], self.relation,
                                              self.current['image'], self.current['cmdline'])
...
class NetworkBehavior(BaseBehavior):
    ...
    def __str__(self):
        return '{} Endpoint({}) on {}: {} -{}-> {}({})'.format(self.__class__.__name__, self.endpoint['uuid'],
                                                               self.date, self.process['image'], self.relation,
                                                               self.network['rhost'], self.network['rip'])

    def get_value(self):
        return '{} -{}-> {}({})'.format(self.process['image'], self.relation, self.network['rhost'],
                                        self.network['rip'])
...
class FileBehavior(BaseBehavior):
    ...
    def __str__(self):
        return '{} Endpoint({}) on {}: {} -{}-> {}'.format(self.__class__.__name__, self.endpoint['uuid'], self.date,
                                                           self.process['image'], self.relation, self.file['path'])
...
class RegistryBehavior(BaseBehavior):
    ...
    def __str__(self):
        return '{} Endpoint({}) on {}: {} -{}-> {} {} {}'.format(self.__class__.__name__, self.endpoint['uuid'],
                                                                 self.date, self.process['image'], self.relation,
                                                                 self.reg['path'], self.reg['key'], self.reg['value'])

    def get_value(self):
        return '{} -{}-> {} {} {}'.format(self.process['image'], self.relation, self.reg['path'], self.reg['key'],
                                          self.reg['value'])
